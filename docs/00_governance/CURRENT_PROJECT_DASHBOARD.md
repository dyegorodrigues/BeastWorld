# BeastWorld — Current Project Dashboard

**Updated:** 2026-09-02  
**Branch:** `chatgpt/beastworld-v5-reconciliation-2026-08-26`  
**Mode:** RECONCILIATION / STORY-FIRST / NOT-CANON  
**Purpose:** one clean current-state front door. Historical pages remain evidence; they are not automatically current.

## Read this first

1. `docs/00_governance/CURRENT_SESSION_POINTER.json`
2. this dashboard
3. `docs/09_reviews/chatgpt/2026-09-02/46_CE05_CREATIVE_SYNTHESIS_SPECIES_TRANSFORMATION_COMBAT_AUTHORIAL_SIGNATURE.md`
4. `docs/09_reviews/chatgpt/2026-09-02/45_CORE_ENGINE_V0_9_CE05_AUTHOR_CORRECTIONS_TWO_ADULT_MODES_DEEP_BODY_TRANSMEDIA.md`
5. `docs/00_governance/CURRENT_CREATIVE_CHECKPOINT_2026-09-01.md`
6. only then load older passes or raw sources on demand.

## Current creative direction

- BeastWorld is a literary epic and story-mother designed for later vertical webcomic, animation and live-action adaptation.
- Biology is the invisible consistency skeleton; magic, beauty, terror, sensuality and martial spectacle remain the audience experience.
- Generated Beasts are new persons born through Beast Matrix × human plasticity × Living World ecology.
- Integrated adulthood and autonomous adulthood are both complete valid modes.
- Autonomous birth opens an own biological body, geography, reproduction and history; it does not rescue a failing symbiosis.
- Autonomous Generated Beasts possess one definitive dual-morph organism: authentic humanoid life and functional bestial Full Form.
- Deep Body / Full Form are current reader-facing candidates; reentrant living anatomy is scholarly/backstage.
- Combat must remain fast, physical, martial, lethal and spatially legible as power grows.
- The Phoenix Elder is a Generated Beast from an older/important generation, not a natural primordial or millennia-old creature.
- The Director remains provisionally 35–40; age is not a power rank.
- Scene-state change is a revision diagnostic after living composition, not a drafting formula.
- The assistant must co-create, research, criticize, challenge, direct, organize and archive—not merely validate.

## Explicitly superseded or rejected

Do not revive as current direction:

- mixed body or temporary biological soma;
- direct energy-to-flesh as baseline;
- transformation healing/reset;
- universal mana bar;
- compulsory autonomous birth;
- universal mature stagnation/regulatory competition inside the human;
- routine post-autonomy return into the former host;
- full human transformation into the complete biological Beast as baseline;
- Phoenix as natural primordial/millennia-old Beast;
- Phoenix transformation staged to test attraction or deliver exposition;
- age as automatic rank;
- “every scene changes state” used as formulaic prose generation.

## Active gate

**CE-05 — Deep Body / Full Form**

Open tests:

1. gravity and inertia;
2. death and remains across morphs;
3. transformation interruption;
4. anti-Quintessence interaction;
5. reproduction and inheritance;
6. maximum scale and ecology.

## Bounded roadmap

| Phase | Planned rounds | Exit |
|---|---:|---|
| close CE-05 | 47–48 | physical leader + reader/narrative proof |
| consolidate Core Engine v0.9 | 49 | CE-01→CE-05 unified |
| character proofs | 50–51 | eight proof roles stress-tested |
| combat proofs | 52 | duel, hunt and battlefield |
| civilizational propagation | 53 | Houses, Academy, medicine, war, law and ecology reconciled |
| Director / Book 1 architecture | 54–55 | dramatic spine, act/season motion and ending pressure |
| visual/adaptation bible | later, parallel | production translation without delaying the literary work |

Rounds are removed when no real fork remains. The workflow is not obligated to consume every number.

## Memory architecture

| Temperature | Load rule | Contents |
|---|---|---|
| HOT | every resumption | pointer, dashboard, current checkpoint, active pass |
| WARM | by cluster | reconciled passes, entity dossiers, gates, horizon map |
| COLD | only for wording/provenance disputes | raw transcripts, PDFs, rejected experiments and historical snapshots |

Current state is rewritten cleanly. Historical evidence is retained with supersession markers rather than deleted or allowed to lead.

## Notion role

Notion is the navigable studio/wiki mirror. Its clean dashboard points to current truth; laboratories retain research history; Source Library preserves provenance. GitHub remains versioned editorial truth.

## Promotion

No discovery in Passes 45–46 has been promoted to canon. Promotion still follows:

> Source preserves → intake classifies → research tests → gate compares → decision records → lore promotes.

